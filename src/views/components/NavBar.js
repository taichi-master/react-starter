...
const MenuItem = ({ menu, showSubmenu, lvl }) => (
  <Link to={ menu.url ? menu.url : '/#' } onClick={ menu.submenu ? showSubmenu : null }>
    ...
    {
      menu.submenu ?
        <span className="caret2" onClick={ showSubmenu }>{ lvl > 0 ? '▸' : '▾' }</span>
        :
        null
    }
  </Link>
...
const Menu = ({ menuItems, className, showSubmenu, lvl = 0 }) => (
  <ul className={ className }>
    {
      menuItems.map(( menu, i ) => {
        ...
        return (
          <li key={ i }>
            <MenuItem menu={ menu } showSubmenu={ showSubmenu } lvl={ lvl } />
            {
              menu.submenu ?
                <Menu menuItems={ menu.submenu } className={ sb.join( ' ' ) } showSubmenu={ showSubmenu } lv={ lvl + 1 } />
                :
                null
            }
          </li>
